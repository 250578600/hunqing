...
//			console.log(selected_id);
			...
			el.attr('value' , selected_id);	
			*/
		}

	}
}
function showLocation(str,province_id,city_id,county_id) {
	if(!province_id)province_id="loc_province";
	if(!city_id)city_id="loc_city";
	if(!county_id)county_id="loc_county";
	province=city=county='';
	if(str){
		l=str.split(',');
		province=parseInt(l[0]);
		if(typeof l[1]!='undefined')city=parseInt(l[1]);
		if(typeof l[2]!='undefined')county=parseInt(l[2]);
	}
function city_change(){
	$('#'+city_id).change(function(){
			c="0,"+$('#'+province_id).val()+","+$('#'+city_id).val();
			if(loc.items[c]){
				$('#'+county_id).empty();
				$('#'+county_id).append("<option value=''>全部区域</option>");
				loc.fillOption(county_id,c);
			}
		});
}
	loc=new Location();
	$('#'+province_id).empty();
	$('#'+province_id).append("<option value=''>省份</option>");
	...
	if(1||county){
		$('#'+county_id).empty();
		$('#'+county_id).append("<option value=''>全部区域</option>");
		loc.fillOption(county_id,"0,"+province+","+city,county);
	}
	$('#'+province_id).change(function(){
		...
		$('#'+county_id).empty();
		$('#'+county_id).append("<option value=''>全部区域</option>");
		loc.fillOption(city_id,"0,"+this.value);
		city_change();
	});
}